...
- 특히 광역 분산 시스템에서 중요한 문제는 다른 기계 간에 프로세스를 이동하는 것이다.
...
- 대신에, 드러났다 $\nearrow$ (뭐가?) 더 정교한 세분화를 갖는 것은 ($\nearrow$ (세분화는) 프로세스당 제어의 다중 스레드의 형성) 분산 애플리케이션을 구축하는 것과 더 나은 성능을 쉽게 얻을 수 있다는 것을
  - **프로세스 (process)** : 완전한 하나의 프로그램이 실행되는 것
...
- IPC 메커니즘의 주요 결함은 통신이 상대적으로 광범위한 문맥 교환을 필요로 한다는 것이다.
...
- 부분들 간 통신은 전적으로 공유 데이터를 사용함으로써 처리된다.
