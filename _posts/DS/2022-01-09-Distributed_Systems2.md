...
- 각 객체는 컴포넌트라 정의한 것과 일치하며, 이런 컴포넌트들은 프로시저 요청 메커니즘을 통해 연결되었다.
- 객체기반 아키텍처는 압축 데이터(객체의 state)와 데이터(객체의 method)가 단일 엔티티로 수행되는 동작을 제공한다.
...
- 객체기반 아키텍처는 캡슐화 서비스가 독립적인 유닛에 들어가는 토대를 형성한다.
  - 캡슐화(encapsulation)는 전체 서비스는 자급자족하는 객체로 실현된다. (이것이 사용될지라도 다른 서비스) 
...
- [^1]: <https://pinelover.tistory.com/187>
- [^2]: <http://wiki.hash.kr/index.php/%ED%94%BC%EC%96%B4>