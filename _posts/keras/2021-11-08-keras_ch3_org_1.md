...
- **층** : 하나 이상의 텐서를 입력을 받아 하나 이상의 텐서를 출력하는 <span style="background-color:yellow">데이터 처리 모듈</span>
...
- **손실 함수**(loss function) (= 목적함수(objective function)) : <span style="background-color:yellow">훈련하는 동안 최소화될 값.</span> 주어진 문제에 대한 성공 지표가 된다.
...
- <span style="background-color:yellow">전형적인 케라스 작업 흐름</span>
...
- 표현 공간의 차원을 <span style="background-color:yellow">'신경망이 내재된 표현을 학습할 때 가질 수 있는 자유도'</span>로 이해할 수 있다.
...
- **에포크 (epoch)** : <span style="background-color:yellow">주어진 데이터를 한번 훈련하는 단위</span>
