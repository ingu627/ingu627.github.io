...
- 일반 DNN은 기본적으로 1차원 형태의 데이터를 사용한다. 때문에 이미지가 입력값이 되는 경우, 이것을 `flatten`시켜서 한줄 데이터로 만들어야 하는데 이 과정에서 이미지의 공간적/지역적 정보(spatial/topological information)가 손실된다. 
...
- **CNN**의 중요 포인트는 이미지 전체보다는 부분을 보는 것, 그리고 이미지의 한 픽셀과 주변 픽셀들의 연관성을 살리는 것이다.
...
- **Dense층**은 입력 특성 공간에 있는 전역 패턴을 학습한다.
- **합성곱층**은 입력 특성 공간에 있는 지역 패턴을 학습한다.
...
2. **컨브넷**은 패턴의 공간적 계층 구조를 학습할 수 있다.
...
- **특성 맵**의 깊이는 네트워크에서 점진적으로 증가하지만(32에서 128까지), 특성 맵의 크기는 감소한다.(150x150에서 7x7)
