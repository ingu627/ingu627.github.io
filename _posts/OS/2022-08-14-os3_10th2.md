...
- 프로세스 협력을 허용하는 환경을 제공하는 데는 다음과 같은 이유가 있다.
  1. **정보 공유(information sharing)** : 여러 응용 프로그램이 동일한 정보에 흥미를 느낄 수 있으므로, 그러한 정보를 병행적(concurrent)으로 접근할 수 있는 환경을 제공해야 한다.
...
- 메시지 전달 모델은 충돌을 회피할 필요가 없기 때문에 적은 양의 데이터를 교환하는 데 유용하다.
  - 메시지 던달은 또한 분산 시스템에서 공유 메모리보다 구현하기 쉽다.
...
- 공유 메모리를 사용하느 프로세스 간 통신에서는 통신하는 프로세스들이 공유 메모리 영역을 구축해야 한다.
...
- 메일박스가 한 프로세스에 의해 소유된다면, 소유자와 메일박스의 사용자를 구분할 수 있다. 
  ...
  - 메일박스의 사용자는 메일박스에 메시지를 송신말 할 수 있는 프로세스이다.
...
- 메시지 전달은 **봉쇄형(blocking)**이거나 **비봉쇄형(nonblocking)** 방식으로 전달된다. 이 두 방식은 각각 **동기식(synchronous)**과 **비동기식(asynchronous)**이라고도 알려져 있다.
...
- send()와 receive()가 모두 봉쇄형일 대, 송신자와 수신자 간에 **랑데부(rendezvous)**를 하게 된다. 
...
- 통신이 직접적이든 간접적이든 간에, 통신하는 프ㅗ세스들에 의해 교환되는 메시지는 임시 큐(queue)에 들어 있다.
- 이러한 큐를 구현하는 방식은 3가지가 있다.
  ...
  2. **유한 용량(bounded capacity)** : 큐는 유한한 길이 n을 가진다. 즉, 최대 n개의 메시지가 그 안에 들어 있을 수 있다. 링크가 만약 만원이면, 송신자는 큐 안에ㅔ 공간이 이용 가능할 때까지 반드시 봉쇄되어야 한다.
  3. **무한 용량(unbounded capacity)** : 큐는 잠재적으로 무한한 길이를 가진다. 따라서 메시지들이 얼마든지 큐 안에서 대기할 수 있다. 송신자는 절대 봉쇄되지 않는다.
...
- Java는 3가지 종류의 소켓을 제공한다. **연결 기반(connection-oriented, TCP)** 소켓은 Socket 클래스로 구현된다. **비연결성(Connectionless, UDP)** 소켓은 DatagramSocket 클래스를 사용한다. MulticastSocket 클래스는 DatagramSocket 클래스의 서브클래스이다.
    ...
    public static void main(String[] args) {
        try {
            ...
            while (true) {
                ...
                서버는 accept() 메소드에서 클라이언트가 연결을 요청할 때까지 봉쇄된다.
...
- 그러면 스텁이 원격 서버의 포트를 찾고 파라미터를 **정돈(marshall)**한다.
