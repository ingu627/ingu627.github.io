...
categories: OS
tag : [OS, 운영체제, 동시 작업, 처리 방식, 컴퓨터, 인터럽트, 커널, 프로그램, 개요, 프로세스]
toc: true
toc_sticky: true
sidebar_main: true
...
- **프로그램(program)** : a set of instructions that tells a computer's hardware to perform a task
- **부트스트랩(bootstrap)** : the first program to run on computer power-on and then loads the operating system
- **인터럽트(interrupt)** : Hard may trigger an interrupt at any time by sending a signal to the CPU, usually by way of the system bus
...
- **Virtualization** : a technology that allow us to abstract the hardware of a single computer into several different execution environments.
