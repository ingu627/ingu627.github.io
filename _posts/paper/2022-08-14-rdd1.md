...
- 이로 인해 데이터 복제, 디스크 I/O 및 직렬화(serialization)로 인해 상당한 오버헤드가 발생하므로 애플리케이션 실행 시간이 많이 소요될 수 있다.
  - **직렬화(serialization)** : 여러 트랜잭션이 동시에 병행(concurrent) 수행되더라도 각 트랜잭션이 하나씩 차례대로 수행되는 것과 같은 데이터베이스의 일관성을 보장하는 수행특성 [^3]
- 따라서 저자들은 데이터 재사용이 필요한 애플리케이션을 위한 전문 프레임워크를 개발했다.
- 본 논문에서는 광범위한 애플리케이션에서 효율적인 데이터 재사용을 가능하게 하는 **복원력있는 분산 데이터 세트(Resilient Distributed Datasets, RDDs)**라는 새로운 추상화를 제안한다.
...
- 이 인터페이스를 사용할 경우 fault tolerance를 제공하는 유일한 방법은 시스템 간에 데이터를 복제하거나 시스템 간에 업데이트를 기록(log)하는 것이다.
  - 비용이 너무 많이 든다.
- RDDs는 많은 데이터 항목에 동일한 작업을 적용하는 거친 입자(coarse-grained) 변환(transformation) (ex. map, filter, join)을 기반으로 인터페이스를 제공한다.
...
- RDD는 1) 안정적인 스토리지 안의 데이터 또는 2) 다른 RDDs에 대한 결정적 연산(deterministic operation)을 통해서만 생성될 수 있다.
  - **결정적(deterministic)** : 예측한 그대로 동작. 어떤 특정한 입력이 들어오면 언제나 독같은 과정을 거쳐서 언제나 똑같은 결과를 내놓는다. [^6]
- 이러한 작업을 RDD의 다른 작업과 구별하기 위해 변환(transformation)이라 부른다.
  - 변환의 예로는 map, filter, join 등이 있다.
- RDD는 다른 데이터 세트(ex. **계통(lineage)**)에서 어떻게 파생되었는지에 대한 충분한 정보를 가지고 있어 안정적인 저장소의 데이터로부터 파티션을 계산한다.
...
- spark는 기본적으로 메모리에 영구적인 RDD를 유지하지만 RAM이 충분하지 않으면 디스크에 영구적인 RDD를 보낼 수 있다.
...
[^3]: [DB - 트랜잭션 - 직렬성 (Serialization) - Char](https://charstring.tistory.com/122)
