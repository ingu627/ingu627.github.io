...
categories: spark
tag : [스파크, spark, 파이썬, 스칼라, scala, 정리, 예제, 실습, lit, equalTo, contains, isin, alias, sql, initcap, trim, lpad, rpad, regexp_extract, regexp_replace, mkString, translate, varargs]
toc: true
toc_sticky: true
sidebar_main: true
...
- 값 목록을 인수로 변환해 함수에 전달할 때는 **varargs**라 불리는 스칼라 고유 기능을 활용한다.
