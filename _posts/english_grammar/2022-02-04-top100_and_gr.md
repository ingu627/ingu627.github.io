...
- **verb**(동사) : a word used to describe an action, state, or occurrence, and forming the main part of the predicate of a sentence
  - **intransitive**(자동사) : expressing action which does not pass over to an object
  - **transitive**(타동사) : taking a direct object
...
- **ed** : (added to nouns to form adjectives) having; possessing; affected by, 2)forming adjectives from phrases consisting of adjective and noun, 3)forming the past tense and past participle of weak verbs, 3)forming participial 
	- **past tense(과거 시제)** : a tense expressing an action that has happened or a state that previouly existed
	- **participle(분사)** : a word formed from a verb and used as an adjective or a noun
	- **past participle(과거 분사)** : the form of a verb which is used in forming perfect and passive tenses and sometimes as an adjective.
