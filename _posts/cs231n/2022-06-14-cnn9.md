...
categories: cs231n
tag : [이미지, cnn, cs231n, 리뷰, 정리, 요약, 정의, 란, 딥러닝, seq2seq, 컨텍스트, rnn, 인코더, 디코더, attention, image caption, 어텐션, alignment, softmax, context vector, self attention, masked self attention, query, key, value, 설명]
toc: true 
toc_sticky: true
sidebar_main: true
...
- features $z_{i,j}$ (HxWxD)와 hidden 상태 $h$ (D)과 입력 데이터이며, 이를 통해 alignment score $e_{i,j}$를 생성한다.
