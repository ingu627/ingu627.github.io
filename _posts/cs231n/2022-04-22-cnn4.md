...
categories: cs231n
tag : [이미지, cnn, cs231n, 리뷰, 정리, 요약, 정의, 란, 딥러닝, 머신러닝, 활성화 맵, 합성곱, filter, stride, 제로 패딩, local connectivity, 풀링, pooling, max pooling, 의미]
toc: true 
toc_sticky: true
sidebar_main: true
...
- 이 필터가 이미지를 슬라이딩(sliding)하면서 공간적으로 내적을 수행한다.
- 필터는 항상 입력의 전체 깊이만큼 확장된다.
...
- 하지만, 뉴런은 **local connectivity**를 가지고 있다.
