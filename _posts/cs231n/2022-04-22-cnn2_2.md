...
categories: cs231n
tag : [이미지, cnn, cs231n, 리뷰, 정리, 요약, 최적화, optimization, loss, 기울기, gradient, 미분, 편미분, 경사하강법, gradient descent, 수렴, step, sgd, stochastic gradient descent, 정의, 란, 미니배치, 딥러닝, 머신러닝, 배치]
toc: true
toc_sticky: true
sidebar_main: true
...
- $L(W)=\frac{1}{N}\sum_{i=1}^NL_i(x_i,y_i,W)+\lambda R(W)$ **(SGD)**
  ...
  - **확률적(Stochastic)** : 전체 학습 데이터 중 랜덤하게 선택된 하나의 데이터로 학습한다는 의미 [^2]
