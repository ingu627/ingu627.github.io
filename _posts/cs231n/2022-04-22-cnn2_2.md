...
categories: cs231n
tag : [이미지, cnn, cs231n, 리뷰, 정리, 요약, 최적화, optimization, loss, 기울기, gradient, 미분, derivative, 편미분, 경사하강법, gradient descent, 수렴, step, sgd, stochastic gradient descent, 정의, 란, 미니배치, 딥러닝, 머신러닝]
toc: true
toc_sticky: true
sidebar_main: true
...
- **그레이디언트(gradient)** : 각 차원으로의 편미분들을 모아놓은 벡터
...
- 즉, **Numerical gradient**를 사용하지 않고, **Analytic gradient**를 사용한다.
  - **Numerical gradient** : 근사, 느림, 쉬운 방법
  - **Analytic gradient** : 정확, 빠름, 실수하기 쉬운(error-prone)
...
- **경사하강법(gradient descent)** : 기울기를 반복적으로 평가한 다음 다음 파라미터 업데이트를 수행하는 절차
...
- **Batch Gradient Descent** : 손실 함수(loss function)을 계산할 때, 전체 학습 데이터셋을 사용하는 것
  - 하나의 step을 위해 전체 데이터를 계산해야 하므로 계산량이 많다.
**Stochastic Gradient Descent** : 손실 함수(loss function)을 계산할 때, 전체 데이터(batch) 대신 일부 데이터의 모음(mini-batch)를 사용하는 것 [^1]
