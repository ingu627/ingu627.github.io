...
categories: cs231n
tag : [이미지, cnn, cs231n, 리뷰, 정리, 요약, 최적화, optimization, loss, 기울기, gradient, 미분, 편미분, 경사하강법, gradient descent, 수렴, step, sgd, stochastic gradient descent, 정의, 란, 미니배치, 딥러닝, 머신러닝, 배치, 의미]
toc: true
toc_sticky: true
sidebar_main: true
...
- **경사 하강법(gradient descent)** : 기울기를 반복적으로 평가한 다음 다음 파라미터 업데이트를 수행하는 절차
