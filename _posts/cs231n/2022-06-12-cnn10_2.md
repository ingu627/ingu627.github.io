...
- Faster R-CNN은 첫 번째 단계가 이미지를 특징짓고 region proposal의 초기 배치를 생성하는 2단계 객체 감지기다. 두 번째 단계는 각 proposal를 개별적으로 처리하여 이러한 영역을 분류하고 세분화하여 최종 예측을 산출하는 것이다.
- 그렇다면 두번째 단계가 정말로 필요한 가에 대해 질문이 있을 수 있다. 
